...
输入数字: " ddr_size_number
    ...
    echo "usage example:./build_tf_a.sh"
    exit 1
fi
...
#指定DDR PHY固件目录
export FWDDR_DIR=$PWD/stm32mp-ddr-phy-A2022.11
...
#编译前先删除上一次编译的部署镜像文件
tf_a_deploydir=$FIP_DEPLOYDIR_ROOT/arm-trusted-firmware
if [ -e "$tf_a_deploydir" ]; then
    rm -rf $tf_a_deploydir
fi
...
if [ $ddr_size_number -eq 1 ]; then
    #编译DDR 1GB配置
    make -f $PWD/../Makefile.sdk TF_A_DEVICETREE=$devicetree_ddr_1GB DEPLOYDIR=$FIP_DEPLOYDIR_ROOT/arm-trusted-firmware all
elif [ $ddr_size_number -eq 2 ]; then
    #编译DDR 2GB配置
    make -f $PWD/../Makefile.sdk TF_A_DEVICETREE=$devicetree_ddr_2GB DEPLOYDIR=$FIP_DEPLOYDIR_ROOT/arm-trusted-firmware all
else
    #非正确编译选项
    echo "error:unsupport"
    exit 1
